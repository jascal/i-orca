...
| `Retention.thy` | **C9** retention by compilation *(from the self-compiling learner, pil `wyly_selfcompile`/`wyly_lm_v5`)* | the preempting cover's covered-domain behavior is independent of the soft function, hence identical at any two training instants; agreement sets on certified subsets are exactly preserved (zero forgetting as semantics); off-domain the cover IS the soft function; with pairwise-disjoint guards the firing rule is sovereign and installation order irrelevant — the obligation the curriculum discharged with token ranges |
| `Arbitration.thy` | **C10** arbitration dominance *(from the support-weighted cover, pil `wyly_lm_v5` sw)* | with per-cell calibrated confidences, argmax arbitration dominates every policy (all fixed tier priorities included); under ε-miscalibration it is within 2ε·(total weight) of optimal — the val-variance lesson as algebra |
