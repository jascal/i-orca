...
| [`PIC_Prune.thy`](PIC_Prune.thy) | source-side pruning certificate: dropping sources with summed incidence (or `β`-budget) under half the margin preserves the decode — the certificate the pruning algorithms gate on |
| [`ROOT`](ROOT) | Isabelle session `KreinPIC` (parent `HOL-Analysis`) |
| [`pic_krein.i.orca.md`](pic_krein.i.orca.md) | the i-orca surface: 39 theorems, each `(rule <lemma>)` |
...
| [`PRUNE.md`](PRUNE.md) | certificate-gated pruning algorithms (greedy magnitude, head/tail, knapsack, iterative, unified budget) with provable within-tolerance outcomes |
...
#   -> all 39 theorems VALID, formal_fraction_static = 1.000
