...
-- goal: (∀m∈{1..PR}. e m / E = 1 / real PR) ∧ (∀A. A ⊆ {1..PR} ⟶ (∑m∈A. e m) / E = real (card A) / real PR)
-- proof DAG (id  [status]  using → claim):
--   s1  [method]  using Epos → ∀m∈{1..PR}. e m / E = 1 / real PR
--   s2  [hammer]  using Epos → ∀A. A ⊆ {1..PR} ⟶ (∑m∈A. e m) / E = real (card A) / real PR
--   s3  [method]  using s1,s2 → (∀m∈{1..PR}. e m / E = 1 / real PR) ∧ (∀A. A ⊆ {1..PR} ⟶ (∑m∈A. e m) / E = real (card A) / real PR)
