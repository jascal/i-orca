...
| [`provable_opt/`](examples/provable_opt) | fieldrun **PROVABLE_OPT** formal arm — PO-T1 (`T_P`-equivalence for the lossless demand/dead-stratum `lastpos` transform) **and** PO-T3 (margin-certified decode invariance + a **tight** boundary), and the **kernel bridge** (rule-level Datalog: the syntactic demand-closure check fieldrun's tool runs ⟹ the semantic one), on a shared general theory + instances | 15/15 surfaced + 3 compiled surfaces, zero `sorry` |
