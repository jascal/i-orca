...
| [`provable_opt/`](examples/provable_opt) | fieldrun **PROVABLE_OPT** formal arm — PO-T1 (`T_P`-equivalence for the lossless demand/dead-stratum `lastpos` transform) **and** PO-T3 (margin-certified decode invariance + a **tight** boundary: necessary + can't-relax witnesses), on a shared general theory + two concrete instances | 10/10 surfaced + 2 compiled surfaces, zero `sorry` |
